...
ckoff = false
if len(hackcheck) > 20 then
   ckoff = (instr(replace(hackcheck,"selection", ""),"select") > 0 and ((instr(hackcheck,"convert") > 0 or instr(hackcheck,"cast") > 0) and (instr(hackcheck,"int") > 0 or instr(hackcheck,"char") > 0)) or (instr(hackcheck,"sysobjects") > 0 or instr(hackcheck,"information_schema") > 0))
