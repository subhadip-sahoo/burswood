...
*/
   function dynCalendar_show()
   {
      ...
      while(i <= numdays){
         ...
         linkHTML = '<a href="javascript: ' + this.callbackFunc + '(' + i + ', ' + (Number(month) + 1) + ', ' + year + '); ' + this.objName + '._hideLayer()"  class=dynCalendar_day>' + (i++) + '</a>';
      ...
      html = '<table border="0" bgcolor="#FFFFFF" cellspacing="0" cellpadding=0>';
      ...
      html += '<td class="dynCalendar_dayname">S</td></tr>';
            html += '<tr><td height=1 colspan=7 bgcolor=#666666></td></tr>';
      html += '<tr>' + ret.join('</tr>\n<tr>') + '</tr>';
         html += '</table>';
...
*/
   function dynCalendar_writeHTML()
   {
      if (is_ie5up || is_nav6up || is_gecko) {
         document.write('<a href="javascript: ' + this.objName + '.show()"><img src="' + this.imagesPath + 'dynCalendar.gif" border="0" width="20" height="18" /></a>');
